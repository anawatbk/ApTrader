...
def local_csv_to_local_parquet_fs(csv_path, filter_tickers: list, _use_thread):
    """
    To be added
    """
    # Parquet file system DIR
    uri = '~/workspace/data/parquet_filesystem/us_equities'
    ...
    curr_date = str(csv_path.stem.split(".")[0])
...
def merging_parquets(year_filter, timeframe='1-min', should_delete_corrupted=False):
    logger.info(f"Optimizing parquets for year={year_filter} timeframe={timeframe}")
    parquet_path = Path(
        f'~/workspace/data/parquet_filesystem/us_equities/year={year_filter}/timeframe={timeframe}').expanduser()
...
if __name__ == '__main__':
    ...
    for year in range(2020, 2021, 1):  # Done 2025,2024, 2023, 2022, 2021

        logger.info(f"Processing year={year}")
        for j, path in tqdm(enumerate(all_csv_path.rglob(f"{year}-*.csv.gz"))):
            local_csv_to_local_parquet_fs(path, None, True)

            # if j != 0 and j % 50 == 0:
            #     merging_parquets(year, "1-min", False)

        # Last optimization before moving the next year
        # corrupted_tickers = merging_parquets(year, "1-min", False)  # True

        # Repair corrupted_tickets
        # local_csv_to_local_parquet_fs(path, corrupted_tickers, False)  # no thread save
        # corrupted_tickers = merging_parquets(year, "1-min", True)
